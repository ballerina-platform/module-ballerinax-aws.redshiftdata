...
isolated function testBasicStatement() returns error? {
    ...
    ExecutionResponse res = check redshiftdata->executeStatement(`SELECT * FROM Users`);
...
isolated function testSessionId() returns error? {
    ...
    ExecutionResponse res1 = check redshiftdata->executeStatement(`SELECT * FROM Users`);
    test:assertTrue(res1.sessionId is string && res1.sessionId != "");

    runtime:sleep(2); // wait for session to establish
    ExecutionResponse res2 = check redshiftdata->executeStatement(`SELECT * FROM Users`,
...
isolated function testExecutionConfig() returns error? {
    ...
    ExecutionResponse res = check redshiftdata->executeStatement(`SELECT * FROM Users`, config);
    test:assertTrue(res.statementId != "");
    check redshiftdata->close();
}
...
isolated function testParameterizedStatement() returns error? {
    ...
    ExecutionResponse res = check redshiftdata->executeStatement(`SELECT * FROM ${tableName}`);
    test:assertTrue(res.statementId != "");
    check redshiftdata->close();
}
...
isolated function testNilParameterizedStatement() returns error? {
    Client redshiftdata = check new Client(testConnectionConfig);
    string? username = ();
    ExecutionResponse res = check redshiftdata->executeStatement(`SELECT * FROM User WHERE username = ${username}`);
    DescriptionResponse descRes = check redshiftdata->describeStatement(res.statementId);
...
isolated function testEmptyStatement() returns error? {
    Client redshiftdata = check new Client(testConnectionConfig);
    ExecutionResponse|Error res = redshiftdata->executeStatement(``);
...
isolated function testWithDbConfigs() returns error? {
    ...
    Client redshiftdata = check new Client(mockConnectionConfig);
    ExecutionResponse res = check redshiftdata->executeStatement(`SELECT * FROM Users`,
        {dbAccessConfig: testDbAccessConfig});
    test:assertTrue(res.statementId != "");
    check redshiftdata->close();
...
isolated function testWithInvalidDbConfigs() returns error? {
    ...
    Client redshiftdata = check new Client(mockConnectionConfig);
    ExecutionResponse|Error res = redshiftdata->executeStatement(`SELECT * FROM Users`);
...
isolated function testWithInvalidStatementName() returns error? {
    Client redshiftdata = check new Client(testConnectionConfig);
    ExecutionResponse|Error res = redshiftdata->executeStatement(`SELECT * FROM Users`, statementName = "");
    test:assertTrue(res is Error);
    if res is Error {
...
isolated function testWithInvalidClusterId() returns error? {
    Client redshiftdata = check new Client(testConnectionConfig);
    ExecutionResponse|Error res = redshiftdata->executeStatement(`SELECT * FROM Users`, dbAccessConfig = {
        id: "",
        database: testDatabaseName
    });
    if res is Error {
